 ...
 * @param {string} reference_id local id for this tx
 * @param {string} callback_url The url 
 * @return {Promise<string>} transfer-id 
 */
async function transfer(sygnaBridgeDomain, api_key, hex_data, transaction, originator_signature, reference_id, callback_url) {
    const url = sygnaBridgeDomain + '/v1/transfer';
    const headers = { "api-key": api_key };
    const params = { hex_data, transaction, originator_signature, reference_id, callback_url };
